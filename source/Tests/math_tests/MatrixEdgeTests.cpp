...
TEST_CASE("Matrix Zero and Identity basics", "[math][matrix]") {
    auto Z = SceneryEditorX::Matrix4x4::Zero();
    auto I = SceneryEditorX::Matrix4x4::Identity();

    // Zero * anything = Zero
    auto T = SceneryEditorX::Matrix4x4::Translation(Vec3(1,2,3));
    auto ZT = Z * T;
    for (int r = 0; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
...
TEST_CASE("Matrix transpose properties", "[math][matrix]") {
    auto R = SceneryEditorX::Matrix4x4::RotationDegrees(Vec3(0, 45.0f, 0));
    auto Rt = SceneryEditorX::Matrix4x4::GetTranspose(R);
    auto Rtt = SceneryEditorX::Matrix4x4::GetTranspose(Rt);
...
static bool approxIdentity(const SceneryEditorX::Matrix4x4& M, float eps = 1e-4f) {
    for (int r = 0; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
...
TEST_CASE("Matrix inverse for simple transform", "[math][matrix][inverse]") {
    auto T = SceneryEditorX::Matrix4x4::Translation(Vec3(1,2,3));
    auto S = SceneryEditorX::Matrix4x4::Scale(Vec3(2, 3, 4));
    auto M = T * S; // affine, invertible
    auto Minv = M.GetInverse();
    auto I = SceneryEditorX::Matrix4x4::Multiply(M, Minv);
