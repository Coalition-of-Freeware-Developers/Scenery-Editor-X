...
TEST_CASE("PerspectiveProjection basic structure", "[math][matrix][projection]") {
    ...
    auto P = SceneryEditorX::Mat4::PerspectiveProjection(aspect, 60.0f, 0.1f, 1000.0f);
...
TEST_CASE("OrthographicProjection basic structure", "[math][matrix][projection]") {
    auto O = SceneryEditorX::Mat4::OrthographicProjection(0.0f, 8.0f, 6.0f, 0.0f, -1.0f, 1.0f);
...
TEST_CASE("Projection invalid inputs don't crash and produce finite or non-finite as-is", "[math][matrix][projection][edge]") {
    ...
    auto Pbad = SceneryEditorX::Mat4::PerspectiveProjection(0.0f, 60.0f, 1.0f, 0.5f);
    // No assertion expected; just touch some entries to ensure it's constructed
    (void)Pbad[0][0];
    auto Obad = SceneryEditorX::Mat4::OrthographicProjection(1.0f, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f);
    (void)Obad[0][0];
}
