...
namespace SceneryEditorX
{
	...
	public:
		...
		inline static Ref<Texture2D> PlayFrameIcon = nullptr;
		inline static Ref<Texture2D> PlayEndIcon = nullptr;
		...
		inline static Ref<Texture2D> GizmoWorldOrientationIcon = nullptr;
		inline static Ref<Texture2D> GizmoLocalOrientationIcon = nullptr;
		...
		static void Init()
		{
			...
			/// Icons (don't forget to .Reset() these in EditorResources::Shutdown())
			...
			/// Viewport (don't forget to .Reset() these in EditorResources::Shutdown())
			PlayIcon = LoadTexture("Viewport/Play.png", "PlayIcon", spec);
			PlayFrameIcon = LoadTexture("Viewport/PlayFrame.png", "PlayFrameIcon", spec);
			PlayEndIcon = LoadTexture("Viewport/PlayEnd.png", "PlayEndIcon", spec);
			...
			GizmoWorldOrientationIcon = LoadTexture("Viewport/World.png", "WorldIcon", spec);
			GizmoLocalOrientationIcon = LoadTexture("Viewport/Local.png", "LocalIcon", spec);

			/// Window (don't forget to .Reset() these in EditorResources::Shutdown())
			...
			/// Content Browser (don't forget to .Reset() these in EditorResources::Shutdown())
			...
			/// Node Graph (don't forget to .Reset() these in EditorResources::Shutdown())
			...
			/// Textures (don't forget to .Reset() these in EditorResources::Shutdown())
		...
		static void Shutdown()
		{
			...
			RotateIcon.Reset();
			ScaleIcon.Reset();
			GizmoWorldOrientationIcon.Reset();
			GizmoLocalOrientationIcon.Reset();
	...
	private:
		...
		{
			return LoadTexture(relativePath, "", std::move(specification));
		}
