...
namespace SceneryEditorX
{
    ...
    public:
        ...
        ShaderResource(const std::string &name, uint32_t set, uint32_t resourceRegister, uint32_t count)
            : name(name), set(set), Register(resourceRegister), count(count) { }

		[[nodiscard]] virtual ~ShaderResource() = default;
