...
/*
	...
	public:
		...
		explicit EditorConsoleSink(uint32_t bufferCapacity) : MessageBufferCapacity_(bufferCapacity), MessageBuffer_(bufferCapacity) {}
    ...
    protected:
        ...
        void sink_it_(const spdlog::details::log_msg &msg) override
        {
            ...
            m_MessageBuffer[MessageBuffer_++] = ConsoleMessage{shortMessage, longMessage, GetMessageFlags(msg.level), std::chrono::system_clock::to_time_t(msg.time)};

            if (MessageBuffer_ == MessageBufferCapacity_)
                flush_();
        }

        void flush_() override
        {
            for (const auto &message : m_MessageBuffer) EditorConsolePanel::PushMessage(message);
    ...
    private:
        uint32_t MessageBufferCapacity_;
        std::vector<ConsoleMessage> m_MessageBuffer;
        uint32_t MessageBuffer_ = 0;
    };
*/
} // namespace SceneryEditor
