...
#include <SceneryEditorX/logging/Console/editor_console_sink.h>
...
std::shared_ptr<spdlog::logger> Log::EditorConsoleLogger_;
...
void Log::Init()
{
	...
	for (auto sink : editorConsoleSinks) sink->set_pattern("%^%v%$");

    // -------------------------------------------------------

    CoreLogger_ = std::make_shared<spdlog::logger>("SceneryEditorX", coreSinks.begin(), coreSinks.end());
    ...
    EditorConsoleLogger_ = std::make_shared<spdlog::logger>("VulkanDebug", editorConsoleSinks.begin(), editorConsoleSinks.end());
    EditorConsoleLogger_->set_level(spdlog::level::trace);
