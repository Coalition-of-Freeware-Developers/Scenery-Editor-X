...
namespace SceneryEditorX
{
	...
	bool Input::IsKeyDown(KeyCode keycode)
	{
		...
		if (!enableImGui)
		{
			auto state = glfwGetKey(static_cast<GLFWwindow*>(Application::Get().GetWindow()->GetNativeWindow()), static_cast<int32_t>(keycode));
	...
	bool Input::IsMouseButtonDown(MouseButton button)
	{
		...
		if (!enableImGui)
		{
			auto state = glfwGetMouseButton(static_cast<GLFWwindow*>(Application::Get().GetWindow()->GetNativeWindow()), static_cast<int32_t>(button));
	...
	{
		glfwSetInputMode(static_cast<GLFWwindow*>(Application::Get().GetWindow()->GetNativeWindow()), GLFW_CURSOR, GLFW_CURSOR_NORMAL + (int)mode);
	...
	{
		return (CursorMode)(glfwGetInputMode(static_cast<GLFWwindow*>(Application::Get().GetWindow()->GetNativeWindow()), GLFW_CURSOR) - GLFW_CURSOR_NORMAL);
	}
