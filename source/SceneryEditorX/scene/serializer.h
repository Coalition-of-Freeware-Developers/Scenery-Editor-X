...
struct Serializer
{
    ...
    Serializer(Json &j, BinaryStorage &storage, int dir, ::AssetManager::AssetManager &manager)
        : j(j), storage(storage), manager(manager), dir(dir)
    {
    }
    ...
    void Serialize(Ref<T> &object)
    {
        if (dir == LOAD)
        {
            DEBUG_ASSERT(j.contains("type") && j.contains("name") && j.contains("uuid"),
                         "Object doens't contain required fields.");
    ...
    template <typename T>
    void Node(const std::string &field, Ref<T> &node, AssetManager::SceneAsset *scene)
    {
        if (dir == SAVE)
        {
            if (node)
