...
void CameraController::Update(Ref<SceneAsset> &scene, Ref<CameraNode> &cam, bool viewportHovered, float deltaTime)
{
    if (!viewportHovered)
    {
        return;
    ...
    Vec3 front = Vec3(view[0][2], view[1][2], view[2][2]);
    ...
    Vec3 targetMoveSpeed = Vec3(.0f);

    Vec3 targetRotationSpeed = Vec3(drag.y, drag.x, 0) * maxRotationSpeed;
    rotationSpeed = glm::mix(rotationSpeed, targetRotationSpeed, rotationSmooth);
    Vec3 newRotation = cam->rotation + rotationSpeed;
    ...
    case CameraNode::CameraMode::Orbit:
        targetMoveSpeed = maxMoveSpeed * Vec3(move.x, move.y, -scroll);
...
Mat4 CameraNode::GetView()
{
    // Implementation
    return Mat4(1.0f); // Placeholder
}
*/
