...
namespace SceneryEditorX::UI
{
	...
	*/
	...
	void RenderWindowOuterBorders(ImGuiWindow* window)
	{
		...
		{
			{.InnerDir = ImVec2(+1, 0),.SegmentN1 = ImVec2(0, 1),.SegmentN2 = ImVec2(0, 0),.OuterAngle = IM_PI * 1.00f }, /// Left
			{.InnerDir = ImVec2(-1, 0),.SegmentN1 = ImVec2(1, 0),.SegmentN2 = ImVec2(1, 1),.OuterAngle = IM_PI * 0.00f }, /// Right
			{.InnerDir = ImVec2(0, +1),.SegmentN1 = ImVec2(0, 0),.SegmentN2 = ImVec2(1, 0),.OuterAngle = IM_PI * 1.50f }, /// Up
			{.InnerDir = ImVec2(0, -1),.SegmentN1 = ImVec2(1, 1),.SegmentN2 = ImVec2(0, 1),.OuterAngle = IM_PI * 0.50f }  /// Down
		};
        ...
        if (int border_held = window->ResizeBorderHeld; border_held != -1)
		{
    ...
    bool UpdateWindowManualResize(ImGuiWindow *window, ImVec2 &newSize, ImVec2 &newPosition)
    {
        ...
        auto CalcWindowSizeAfterConstraint = [](ImGuiWindow *window, const ImVec2 &size_desired) {
            ...
            {
                ImGuiWindow *window_for_height = (window->DockNodeAsHost && window->DockNodeAsHost->VisibleWindow)
        ...
        auto CalcWindowAutoFitSize = [CalcWindowSizeAfterConstraint](ImGuiWindow *window, const ImVec2 &size_contents) {
            ...
            if (window->Flags & ImGuiWindowFlags_Tooltip)
                return size_desired; /// Tooltip always resize
            ...
            if (const int monitor_idx = window->ViewportAllowPlatformMonitorExtend;
                monitor_idx >= 0 && monitor_idx < g.PlatformIO.Monitors.Size)
                avail_size = g.PlatformIO.Monitors[monitor_idx].WorkSize;

            ImVec2 size_auto_fit = ImClamp(size_desired,
                                           size_min,
                                           ImMax(size_min,
        ...
        if (const bool handle_borders_and_resize_grips = (window->DockNodeAsHost || !window->DockIsActive);
            !handle_borders_and_resize_grips || window->Collapsed)
            return false;
        ...
        ///This was moved to inside brackets in line 305
        ...
        static constexpr ImGuiResizeBorderDef resize_border_def[4] = {
            {.InnerDir = ImVec2(+1, 0),
             .SegmentN1 = ImVec2(0, 1),
             .SegmentN2 = ImVec2(0, 0),
             .OuterAngle = IM_PI * 1.00f}, /// Left
            {.InnerDir = ImVec2(-1, 0),
             .SegmentN1 = ImVec2(1, 0),
             .SegmentN2 = ImVec2(1, 1),
             .OuterAngle = IM_PI * 0.00f}, /// Right
            {.InnerDir = ImVec2(0, +1),
             .SegmentN1 = ImVec2(0, 0),
             .SegmentN2 = ImVec2(1, 0),
             .OuterAngle = IM_PI * 1.50f}, /// Up
            {.InnerDir = ImVec2(0, -1),
             .SegmentN1 = ImVec2(1, 1),
             .SegmentN2 = ImVec2(0, 1),
             .OuterAngle = IM_PI * 0.50f} /// Down
        };
        ...
        static constexpr ImGuiResizeGripDef resize_grip_def[4] = {
            {.CornerPosN = ImVec2(1, 1), .InnerDir = ImVec2(-1, -1), .AngleMin12 = 0, .AngleMax12 = 3}, /// Lower-right
            {.CornerPosN = ImVec2(0, 1), .InnerDir = ImVec2(+1, -1), .AngleMin12 = 3, .AngleMax12 = 6}, /// Lower-left
            {.CornerPosN = ImVec2(0, 0),
             .InnerDir = ImVec2(+1, +1),
             .AngleMin12 = 6,
             .AngleMax12 = 9}, /// Upper-left (Unused)
            {.CornerPosN = ImVec2(1, 0), .InnerDir = ImVec2(-1, +1), .AngleMin12 = 9, .AngleMax12 = 12}
            /// Upper-right (Unused)
        };

        auto CalcResizePosSizeFromAnyCorner = [CalcWindowSizeAfterConstraint](ImGuiWindow *window,
                                                                              const ImVec2 &corner_target,
                                                                              const ImVec2 &corner_norm,
                                                                              ImVec2 *out_pos,
                                                                              ImVec2 *out_size) {
            ImVec2 pos_min = ImLerp(corner_target, window->Pos, corner_norm); /// Expected window upper-left
            ImVec2 pos_max = ImLerp({window->Pos.x + window->Size.x, window->Pos.y + window->Size.y},
                                    corner_target,
        ...
        static constexpr float WINDOWS_HOVER_PADDING =
            4.0f; /// Extend outside window for hovering/resizing (maxed with TouchPadding) and inside windows for borders. Affect FindHoveredWindow().
        static constexpr float WINDOWS_RESIZE_FROM_EDGES_FEEDBACK_TIMER =
        ...
        if (const bool clip_with_viewport_rect = !(g.IO.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) ||
        ...
        for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
        {
            ...
            const ImVec2 corner =
                ImLerp(window->Pos, {window->Pos.x + window->Size.x, window->Pos.y + window->Size.y}, def.CornerPosN);
            ...
            ImGui::ButtonBehavior(resize_rect,
                                  resize_grip_id,
                                  &hovered,
                                  &held,
            ...
            if (resize_grip_n == 0 || held || hovered)
                resize_grip_col[resize_grip_n] = ImGui::GetColorU32(held      ? ImGuiCol_ResizeGripActive
    ...
    */
	...
	void DrawShadow(const Ref<Texture2D>& shadowImage, int radius, ImVec2 rectMin, ImVec2 rectMax, float alphMultiplier, float lengthStretch, bool drawLeft, bool drawRight, bool drawTop, bool drawBottom)
	{
	...
	void DrawShadow(const Ref<Texture2D>& shadowImage, int radius, ImRect rectangle, float alphMultiplier, float lengthStretch, bool drawLeft, bool drawRight, bool drawTop, bool drawBottom)
	{
	...
	void DrawShadow(const Ref<Texture2D>& shadowImage, int radius, float alphMultiplier, float lengthStretch, bool drawLeft, bool drawRight, bool drawTop, bool drawBottom)
	{
	...
	void DrawShadowInner(const Ref<Texture2D>& shadowImage, int radius, ImVec2 rectMin, ImVec2 rectMax, float alpha, float lengthStretch, bool drawLeft, bool drawRight, bool drawTop, bool drawBottom)
	{
		const float widthOffset = lengthStretch;
	...
	void DrawShadowInner(const Ref<Texture2D>& shadowImage, int radius, ImRect rectangle, float alpha, float lengthStretch, bool drawLeft, bool drawRight, bool drawTop, bool drawBottom)
	{
		DrawShadowInner(shadowImage, radius, rectangle.Min, rectangle.Max, alpha, lengthStretch, drawLeft, drawRight, drawTop, drawBottom);
	};

	void DrawShadowInner(const Ref<Texture2D>& shadowImage, int radius, float alpha, float lengthStretch, bool drawLeft, bool drawRight, bool drawTop, bool drawBottom)
	{
    ...
    bool FakeComboBoxButton(const char* label, const char* preview_value, ImGuiComboFlags flags)
	{
		...
		if (preview_value != nullptr && !(flags & ImGuiComboFlags_NoPreview))
		{
			...
			ImGui::RenderTextClipped(bb.Min + style.FramePadding, ImVec2(value_x2, bb.Max.y), preview_value, nullptr, nullptr);
		}
		if (label_size.x > 0)
	...
	void DrawButtonImage(const Ref<Texture2D> &imageNormal, const Ref<Texture2D> &imageHovered, const Ref<Texture2D> &imagePressed, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImVec2 rectMin, ImVec2 rectMax, ImVec2 uv0, ImVec2 uv1)
	{
		auto* drawList = ImGui::GetWindowDrawList();
	...
	void DrawButtonImage(const Ref<Image2D> &imageNormal, const Ref<Image2D> &imageHovered, const Ref<Image2D> &imagePressed, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImVec2 rectMin, ImVec2 rectMax, ImVec2 uv0, ImVec2 uv1)
	{
		auto* drawList = ImGui::GetWindowDrawList();
		if (ImGui::IsItemActive())
			drawList->AddImage(GetTextureID(imagePressed), rectMin, rectMax, uv0, uv1, tintPressed);
