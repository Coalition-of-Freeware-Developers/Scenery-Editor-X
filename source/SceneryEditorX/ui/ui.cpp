...
#include "SceneryEditorX/renderer/vulkan/sampler.h"
...
namespace SceneryEditorX::UI
{
	...
	bool GUI::CreateDescriptorPool()
	{
		...
		if (vkCreateDescriptorPool(device->GetLogicalDevice(), &poolInfo, nullptr, &imguiPool) != VK_SUCCESS)
		{
	...
	{
		/*
		const auto window = static_cast<SDL_Window * *>(Application::Get().GetWindow().GetWindow());
		/// Get content scale from SDL
		float xScale, yScale;
		SDL_GetWindowSize(window, &xScale, &yScale);
		contentScaleFactor = xScale;

		/// Get monitor DPI info if available
		if (SDL_DisplayMode *mode = SDL_GetCurrentDisplayMode(0))
		{
			float xDpi, yDpi;
			SDL_GetDisplayDPI(0, &xDpi, &yDpi, nullptr);
			dpiFactor = xDpi;
		}
		else
			dpiFactor = xScale;
	...
	*/
	...
	void GUI::CleanUp()
	{
		const auto device = RenderContext::Get()->GetDevice();
		if (!initialized)
			return;
