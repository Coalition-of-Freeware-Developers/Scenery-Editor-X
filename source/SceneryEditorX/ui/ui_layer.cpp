...
namespace SceneryEditorX::UI
{
	...
	void UILayer::OnAttach()
	{
		...
		Renderer::Submit([instance]()
		{
			...
			SDL_Window *window = static_cast<SDL_Window *>(app.GetWindow().GetWindow());
			...
			SEDX_VK_RESULT_ASSERT(vkCreateDescriptorPool(device->GetLogicalDevice(), &pool_info, nullptr, &descriptorPool));

			// Setup Platform/Renderer bindings
			ImGui_ImplSDL3_InitForVulkan(window);
			ImGui_ImplVulkan_InitInfo init_info = {};
			init_info.Instance = RenderContext::GetInstance();
			init_info.PhysicalDevice = RenderContext::Get()->GetDevice()->GetPhysicalDevice();
			init_info.Device = device->GetLogicalDevice();
			init_info.QueueFamily = RenderContext::Get()->GetDevice()->GetPhysicalDevice()->GetQueueFamilyIndices().graphics;
			init_info.Queue = RenderContext::Get()->GetDevice()->GetQueueManager()->GetGraphicsQueue();
			...
			{
				s_ImGuiCommandBuffers[i] = RenderContext::Get()->CreateSecondaryCommandBuffer("UI Secondary CommandBuffer");
			}
		});
	}
	...
	void UILayer::OnDetach()
	{
		Renderer::Submit([]()
		{
			...
			SEDX_VK_RESULT_ASSERT(vkDeviceWaitIdle(device->GetLogicalDevice()));
	...
	void UILayer::End()
	{
		...
		SEDX_VK_RESULT_ASSERT(vkBeginCommandBuffer(drawCommandBuffer, &drawCmdBufInfo));
		...
		SEDX_VK_RESULT_ASSERT(vkBeginCommandBuffer(s_ImGuiCommandBuffers[commandBufferIndex], &cmdBufInfo));
		...
		SEDX_VK_RESULT_ASSERT(vkEndCommandBuffer(s_ImGuiCommandBuffers[commandBufferIndex]));
		...
		SEDX_VK_RESULT_ASSERT(vkEndCommandBuffer(drawCommandBuffer));
