...
namespace SceneryEditorX::UI
{
	...
	public:
		...
		ScopedColourStack(ImGuiCol firstColourID, ColourType firstColour, OtherColours&& ... otherColourPairs) : m_Count((sizeof... (otherColourPairs) / 2) + 1)
		{
			static_assert ((sizeof... (otherColourPairs) & 1u) == 0, "ScopedColourStack constructor expects a list of pairs of colour IDs and colours as its arguments");
	...
	public:
		...
		ScopedStyleStack(ImGuiStyleVar firstStyleVar, ValueType firstValue, OtherStylePairs&& ... otherStylePairs) : m_Count((sizeof... (otherStylePairs) / 2) + 1)
		{
	...
	public:
		...
		{
			//SEDX_CORE_VERIFY(!(flags & ImGuiItemFlags_Disabled), "We shouldn't use ImGuiItemFlags_Disabled! Use UI::BeginDisabled / UI::EndDisabled instead. It will handle visuals for you.");
	...
	inline void SetTooltip(std::string_view text, float delayInSeconds = 0.1f, bool allowWhenDisabled = true, ImVec2 padding = ImVec2(5, 5))
	{
		if (IsItemHovered(delayInSeconds, allowWhenDisabled ? ImGuiHoveredFlags_AllowWhenDisabled : 0))
		{
			...
			ScopedColour textCol(ImGuiCol_Text, Colors::Theme::textBrighter);
	...
	{
		/// NOTE: Cheeky hack to prevent ImGui from asserting (required due to the nature of UI::BeginDisabled)
		if (GImGui->DisabledStackSize > 0)
			ImGui::EndDisabled();
	}

	typedef int OutlineFlags;
	enum OutlineFlags_ : uint8_t
	{
	...
	bool PropertyDropdown(const char* label, const std::vector<std::string>& options, int32_t optionCount, int32_t* selected, GetColFunction getColourFunction)
	{
	...
	void DrawButtonImage(const Ref<Image2D> &imageNormal, const Ref<Image2D> &imageHovered, const Ref<Image2D> &imagePressed, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImVec2 rectMin, ImVec2 rectMax,
		ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f));

	inline void DrawButtonImage(const Ref<Texture2D> &imageNormal, const Ref<Texture2D> &imageHovered, const Ref<Texture2D> &imagePressed, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImRect rectangle, ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f))
	{
	...
	inline void DrawButtonImage(const Ref<Image2D> &imageNormal, const Ref<Image2D> &imageHovered, const Ref<Image2D> &imagePressed, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImRect rectangle, ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f))
	{
		DrawButtonImage(imageNormal, imageHovered, imagePressed, tintNormal, tintHovered, tintPressed, rectangle.Min, rectangle.Max, uv0, uv1);
	}

	inline void DrawButtonImage(const Ref<Texture2D> &image, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImVec2 rectMin, ImVec2 rectMax, ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f))
	{
		DrawButtonImage(image, image, image, tintNormal, tintHovered, tintPressed, rectMin, rectMax, uv0, uv1);
	}

	inline void DrawButtonImage(const Ref<Texture2D> &image, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImRect rectangle, ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f))
	{
		DrawButtonImage(image, image, image, tintNormal, tintHovered, tintPressed, rectangle.Min, rectangle.Max, uv0, uv1);
	}

	inline void DrawButtonImage(const Ref<Image2D> &image, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImVec2 rectMin, ImVec2 rectMax, ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f))
	{
		DrawButtonImage(image, image, image, tintNormal, tintHovered, tintPressed, rectMin, rectMax, uv0, uv1);
	}

	inline void DrawButtonImage(const Ref<Image2D> &image, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImRect rectangle, ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f))
	{
		DrawButtonImage(image, image, image, tintNormal, tintHovered, tintPressed, rectangle.Min, rectangle.Max, uv0, uv1);
	}

	inline void DrawButtonImage(const Ref<Texture2D> &imageNormal, const Ref<Texture2D> &imageHovered, const Ref<Texture2D> &imagePressed, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f))
	{
	...
	inline void DrawButtonImage(const Ref<Texture2D> &image, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f))
	{
		DrawButtonImage(image, image, image, tintNormal, tintHovered, tintPressed, ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), uv0, uv1);
	}

	inline void DrawButtonImage(const Ref<Image2D> &image, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed, ImVec2 uv0 = ImVec2(0.0f, 0.0f), ImVec2 uv1 = ImVec2(1.0f, 1.0f))
	{
		DrawButtonImage(image, image, image, tintNormal, tintHovered, tintPressed, ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), uv0, uv1);
	}
	...
	inline int FormatString(char* buf, size_t buf_size, const char* fmt, ...)
	{
		...
		if (buf == nullptr)
			return w;
		if (w == -1 || std::cmp_greater_equal(w, buf_size))
	...
	inline bool DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
	{
		...
		const bool hovered = ImGui::ItemHoverable(frame_bb, id, temp_input_allowed ? ImGuiItemFlags_Inputable : 0);
		...
		if (temp_input_is_active)
		{
			...
			return ImGui::TempInputScalar(frame_bb, id, label, data_type, p_data, format, is_clamp_input ? p_min : nullptr, is_clamp_input ? p_max : nullptr);
		}
	...
	inline bool DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed = 1.0f, const void* p_min = nullptr, const void* p_max = nullptr, const char* format = nullptr, ImGuiSliderFlags flags = 0)
	{
	...
	inline bool InputText(const char* label, std::string* value, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = nullptr, void* user_data = nullptr)
	{
	...
	inline bool InputTextMultiline(const char* label, std::string* value, const ImVec2& size = ImVec2(0, 0), ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = nullptr, void* user_data = nullptr)
	{
