...
namespace SceneryEditorX
{
        ...
        if (!m_Specification.existingFramebuffer)
        {
            ...
            for (auto& attachmentSpec : m_Specification.attachments.Attachments)
			{
				...
				else if (Utils::IsDepthFormat(attachmentSpec.Format))
				{
					...
					spec.width = static_cast<uint32_t>(m_Width * m_Specification.scale);
					spec.height = static_cast<uint32_t>(m_Height * m_Specification.scale);
	...
	void Framebuffer::Resize(uint32_t width, uint32_t height, bool forceRecreate)
	{
        ...
        {
            instance->m_Width = static_cast<uint32_t>(width * instance->m_Specification.scale);
            instance->m_Height = static_cast<uint32_t>(height * instance->m_Specification.scale);
        ...
        if (m_Framebuffer)
        {
            ...
            if (!m_Specification.existingFramebuffer)
            {
                ...
                if (m_DepthAttachmentImage)
                {
                    ...
                    if (!m_Specification.existingImages.contains(static_cast<uint32_t>(m_Specification.attachments.Attachments.size()) - 1))
