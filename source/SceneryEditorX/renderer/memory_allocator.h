...
namespace SceneryEditorX
{
    ...
    public:
        ...
        void FrameTick(uint64_t frameCount) const;
        void ResetStats();
    ...
    private:
        std::string m_Tag = "default tag";
        std::vector<VmaAllocation> m_DefragCandidates;
        VmaDefragmentationContext m_DefragContext = nullptr;
        AllocationStrategy m_CurrentStrategy = AllocationStrategy::DEFAULT;

        // ---------------------------------------------------------

        // Fixed-size pools for common allocation sizes
        std::unordered_map<VkDeviceSize, MemoryPool> m_BufferPools;
        std::unordered_map<VkDeviceSize, MemoryPool> m_ImagePools;
