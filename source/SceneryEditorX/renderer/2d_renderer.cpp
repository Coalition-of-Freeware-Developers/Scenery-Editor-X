...
namespace SceneryEditorX
{
	...
	/*
	Renderer2D::Renderer2D(const Renderer2DSpecification& specification) : c_MaxVertices(specification.MaxQuads * 4), c_MaxIndices(specification.MaxQuads * 6),
		    c_MaxLineVertices(specification.MaxLines * 2), c_MaxLineIndices(specification.MaxLines * 2), m_Specification(specification)
	{
		Init();
	}
	...
	void Renderer2D::Init()
	{
		...
		/*
		{
...

			{
                ...
                lineSpec.Pipeline = CreateRef<Pipeline>(pipelineSpecification);
				m_LinePass = CreateRef<RenderPass>(lineSpec);
				m_LinePass->AddInput("Camera", m_UBSCamera);
				SEDX_CORE_VERIFY(m_LinePass->Validate());
				m_LinePass->Bake();
			}
		...
		///< Text
		{
			...
			m_TextMaterial = CreateRef<Material>(pipelineSpecification.shader);
		...
		///< Circles
		{
            ...
            m_CirclePipeline = CreateRef<Pipeline>(pipelineSpecification);
            m_CircleMaterial = CreateRef<Material>(pipelineSpecification.shader);
	...
	void Renderer2D::BeginScene(const Mat4& viewProj, const Mat4& view, bool depthTest)
	{
		uint32_t frameIndex = Renderer::GetCurrentFrameIndex();

        if(const bool updatedAnyShaders = Renderer::UpdateDirtyShaders())
		{
		...
		for (auto &m_FontTextureSlot : m_FontTextureSlots)
            m_FontTextureSlot = nullptr;
	}
	...
	void Renderer2D::EndScene()
	{
		...
		for (uint32_t i = 0; i <= m_QuadBufferWriteIndex; i++)
		{
			...
			if (dataSize)
			{
				...
				for (uint32_t arrayIndex = 0; arrayIndex < m_TextureSlots.size(); arrayIndex++)
				{
					if (m_TextureSlots[arrayIndex])
						m_QuadMaterial->Set("u_Textures", m_TextureSlots[arrayIndex], arrayIndex);
					else
						m_QuadMaterial->Set("u_Textures", m_WhiteTexture, arrayIndex);
				}
		...
		for (uint32_t i = 0; i <= m_TextBufferWriteIndex; i++)
		{
			...
			if (dataSize)
			{
				...
				for (uint32_t arrayIndex = 0; arrayIndex < m_FontTextureSlots.size(); arrayIndex++)
				{
					if (m_FontTextureSlots[arrayIndex])
						m_TextMaterial->Set("u_FontAtlases", m_FontTextureSlots[arrayIndex], arrayIndex);
					else
						m_TextMaterial->Set("u_FontAtlases", m_WhiteTexture, arrayIndex);
				}
		...
		for (uint32_t i = 0; i <= m_LineBufferWriteIndex; i++)
		{
			...
			if (dataSize)
			{
				...
				Renderer::BeginFrame(m_RenderCommandBuffer, m_LinePass);
				Renderer::RenderGeometry(m_RenderCommandBuffer, m_LinePass->GetSpecification().Pipeline, m_LineMaterial, m_LineVertexBuffers[i][frameIndex], m_LineIndexBuffer, Mat4(1.0f), indexCount);
				Renderer::EndFrame(m_RenderCommandBuffer);
		...
		for (uint32_t i = 0; i <= m_LineOnTopBufferWriteIndex; i++)
		{
			...
			if (dataSize)
			{
				...
				Renderer::BeginFrame(m_RenderCommandBuffer, m_LinePass);
				Renderer::RenderGeometry(m_RenderCommandBuffer, m_LinePass->GetSpecification().Pipeline, m_LineMaterial, m_LineOnTopVertexBuffers[i][frameIndex], m_LineOnTopIndexBuffer, Mat4(1.0f), indexCount);
				Renderer::EndFrame(m_RenderCommandBuffer);
		...
		if (dataSize)
		{
			...
			///< TODO: Renderer::RenderGeometry(m_RenderCommandBuffer, m_CirclePipeline, m_CircleMaterial, m_CircleVertexBuffer[frameIndex], m_QuadIndexBuffer, Mat4(1.0f), m_CircleIndexCount);
	...
	void Renderer2D::SetTargetFramebuffer(const Ref<Framebuffer> &framebuffer)
	{
		...
		{
			{
				PipelineData pipelineSpec = m_QuadPass->GetSpecification().Pipeline->GetSpecification();
				pipelineSpec.dstFramebuffer = framebuffer;
                RenderSpec &renderpassSpec = m_QuadPass->GetSpecification();
                renderpassSpec.Pipeline = CreateRef<Pipeline>(pipelineSpec);
			}

			{
				PipelineData pipelineSpec = m_LinePass->GetSpecification().Pipeline->GetSpecification();
				pipelineSpec.dstFramebuffer = framebuffer;
				RenderSpec& renderpassSpec = m_LinePass->GetSpecification();
                renderpassSpec.Pipeline = CreateRef<Pipeline>(pipelineSpec);
			}

			{
				PipelineData pipelineSpec = m_TextPass->GetSpecification().Pipeline->GetSpecification();
				pipelineSpec.dstFramebuffer = framebuffer;
				RenderSpec& renderpassSpec = m_TextPass->GetSpecification();
                renderpassSpec.Pipeline = CreateRef<Pipeline>(pipelineSpec);
			}
		}
	}
	...
	Renderer2D::QuadVertex*& Renderer2D::GetWriteableQuadBuffer()
	{
		...
		{
			AddQuadBuffer();
			m_QuadVertexBufferPtr.emplace_back(); ///< TODO: check
	...
	Renderer2D::LineVertex*& Renderer2D::GetWriteableLineBuffer(const bool onTop)
	{
		...
		if (onTop)
		{
			...
			{
				AddLineBuffer(onTop);
				m_LineOnTopVertexBufferPtr.emplace_back(); ///< TODO: check
        ...
        {
            AddLineBuffer(onTop);
            m_LineVertexBufferPtr.emplace_back(); ///< TODO: check
	...
	Renderer2D::TextVertex*& Renderer2D::GetWriteableTextBuffer()
	{
		...
		{
			AddTextBuffer();
			m_TextVertexBufferPtr.emplace_back(); ///< TODO: check
	...
	Renderer2D::CircleVertex*& Renderer2D::GetWriteableCircleBuffer()
	{
		...
		{
			AddCircleBuffer();
			m_CircleVertexBufferPtr.emplace_back(); ///< TODO: check
	...
	void Renderer2D::DrawQuad(const Mat4& transform, const Ref<Texture2D>& texture, const float tilingFactor, const Vec4& tintColor, Vec2 uv0, Vec2 uv1)
	{
		constexpr size_t quadVertexCount = 4;
	...
	void Renderer2D::DrawQuad(const Vec2& position, const Vec2& size, const Ref<Texture2D>& texture, float tilingFactor, const Vec4& tintColor, Vec2 uv0, Vec2 uv1)
	{
		DrawQuad({ position.x, position.y, 0.0f }, size, texture, tilingFactor, tintColor, uv0, uv1);
	}

	void Renderer2D::DrawQuad(const Vec3& position, const Vec2& size, const Ref<Texture2D>& texture, float tilingFactor, const Vec4& tintColor, Vec2 uv0, Vec2 uv1)
	{
		float textureIndex = 0.0f;
		for (uint32_t i = 1; i < m_TextureSlotIndex; i++)
		{
	...
	void Renderer2D::DrawQuadBillboard(const Vec3& position, const Vec2& size, const Vec4& color)
	{
        constexpr float textureIndex = 0.0f; ///< White Texture
        constexpr float tilingFactor = 1.0f;

		const Vec3 camRightWS = { m_CameraView[0][0], m_CameraView[1][0], m_CameraView[2][0] };
		const Vec3 camUpWS = { m_CameraView[0][1], m_CameraView[1][1], m_CameraView[2][1] };
	...
	void Renderer2D::DrawQuadBillboard(const Vec3& position, const Vec2& size, const Ref<Texture2D>& texture, float tilingFactor, const Vec4& tintColor)
	{
		float textureIndex = 0.0f;
		for (uint32_t i = 1; i < m_TextureSlotIndex; i++)
		{
	...
	void Renderer2D::DrawRotatedQuad(const Vec2& position, const Vec2& size, const float rotation, const Vec4& color)
	{
	...
	void Renderer2D::FillCircle(const Vec2& position, float radius, const Vec4& color, const float thickness)
	{
	...
	void Renderer2D::DrawTransform(const Mat4& transform, float scale /*= 1.0f#1#, const bool onTop)
	{
	...
	void Renderer2D::DrawCircle(const Mat4& transform, const Vec4& color, const bool onTop)
	{
	...
	void Renderer2D::DrawAABB(const Utils::AABB &aabb, const Mat4& transform, const Vec4& color /*= Vec4(1.0f)#1#, const bool onTop)
	{
		...
		{
			transform * Vec4 { aabb.Min.x, aabb.Min.y, aabb.Max.z, 1.0f },
			transform * Vec4 { aabb.Min.x, aabb.Max.y, aabb.Max.z, 1.0f },
			transform * Vec4 { aabb.Max.x, aabb.Max.y, aabb.Max.z, 1.0f },
			transform * Vec4 { aabb.Max.x, aabb.Min.y, aabb.Max.z, 1.0f },

			transform * Vec4 { aabb.Min.x, aabb.Min.y, aabb.Min.z, 1.0f },
			transform * Vec4 { aabb.Min.x, aabb.Max.y, aabb.Min.z, 1.0f },
			transform * Vec4 { aabb.Max.x, aabb.Max.y, aabb.Min.z, 1.0f },
			transform * Vec4 { aabb.Max.x, aabb.Min.y, aabb.Min.z, 1.0f }
		};
...
*/
	...
	void Renderer2D::DrawString(const std::string& string, const Ref<Font>& font, const Mat4& transform, float maxWidth, const Vec4& color, float lineHeightOffset, float kerningOffset)
	{
		...
		///< TODO: this isn't really ideal, but we need to iterate through UTF-8 code points
		std::u32string utf32string = To_UTF32(string);

		Ref<Texture2D> fontAtlas = font->GetFontAtlas();
		SEDX_CORE_ASSERT(fontAtlas);
		...
		std::vector<int> nextLines;
		{
			...
			for (int i = 0; i < utf32string.size(); i++)
			{
				...
				if (character != ' ')
				{
					...
					Vec2 quadMin((float)pl, (float)pb);
					Vec2 quadMax((float)pr, (float)pt);
	...
	*/
	...
	Renderer2D::DrawStatistics Renderer2D::GetDrawStats() const
    {
		return m_DrawStats;
	}

	Renderer2D::MemoryStatistics Renderer2D::GetMemoryStats() const
    {
