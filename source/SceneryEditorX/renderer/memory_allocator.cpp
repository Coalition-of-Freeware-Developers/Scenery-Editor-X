...
namespace SceneryEditorX
{
    ...
    VkDeviceSize MemoryAllocator::m_CustomBufferSize = DEFAULT_CUSTOM_BUFFER_SIZE;
     ...
     */
    MemoryAllocator::MemoryAllocator(std::string tag) : m_Tag(std::move(tag)), m_CurrentStrategy() { }
    ...
    {
        std::scoped_lock lock(other.m_AllocationMutex);

        // Move data from other
        m_Tag = std::move(other.m_Tag);
        m_CurrentStrategy = other.m_CurrentStrategy;
        m_DefragContext = other.m_DefragContext;
        m_DefragCandidates = std::move(other.m_DefragCandidates);
        m_BufferPools = std::move(other.m_BufferPools);
        m_ImagePools = std::move(other.m_ImagePools);
        m_CustomBufferAlignment = other.m_CustomBufferAlignment;
        m_MemoryWarningThreshold = other.m_MemoryWarningThreshold;

        // Reset other's state to a valid empty state
        other.m_DefragContext = nullptr;
        other.m_DefragCandidates.clear();
        other.m_BufferPools.clear();
        other.m_ImagePools.clear();
        other.m_CurrentStrategy = AllocationStrategy::DEFAULT;
        other.m_CustomBufferAlignment = 0;
        other.m_MemoryWarningThreshold = 0.9f;
    }
    ...
    MemoryAllocator &MemoryAllocator::operator=(MemoryAllocator &&other) noexcept
    {
        if (this != &other)
        {
            ...
            std::scoped_lock lock(m_AllocationMutex, other.m_AllocationMutex);
    ...
    {
        std::scoped_lock lock(this->m_AllocationMutex);
        ...
        if (m_DefragContext != nullptr)
        {
            SEDX_CORE_WARN("Defragmentation already in progress, ending previous session first");
            EndDefragmentation();
        }

        m_DefragCandidates.clear();	// Clear any previous candidates list
    ...
    void MemoryAllocator::EndDefragmentation()
    {
        ...
        if (VkResult result = vmaBeginDefragmentation(memAllocatorData->allocator, &defragInfo, &m_DefragContext); result != VK_SUCCESS)
        {
            SEDX_CORE_ERROR("Failed to begin memory defragmentation, error: {}", static_cast<int>(result));
            m_DefragContext = nullptr;
            m_DefragCandidates.clear();
            return;
        }

        // End the defragmentation operation
        vmaEndDefragmentation(memAllocatorData->allocator, m_DefragContext, &defragStats);
    ...
    void MemoryAllocator::MarkForDefragmentation(VmaAllocation allocation)
    {
        ...
        if (std::ranges::find(m_DefragCandidates, allocation) != m_DefragCandidates.end())
        ...
        m_DefragCandidates.push_back(allocation);

        // Log only occasionally to prevent spam
        if (m_DefragCandidates.size() % 100 == 1 || m_DefragCandidates.size() < 5)
        {
            ...
            SEDX_CORE_INFO("Marked {} allocation of size {} for defragmentation ({} total marked)", typeStr, FILE_SIZE(allocatedSize), m_DefragCandidates.size());
        }
    }
    ...
    VkDeviceSize MemoryAllocator::GetCustomBufferSize()
    {
        ...
        return m_CustomBufferSize ? m_CustomBufferSize : DEFAULT_CUSTOM_BUFFER_SIZE;
    }
    ...
    bool MemoryAllocator::SetCustomBufferSize(const VkDeviceSize size, const VulkanDevice &device)
    {
        ...
        {
            if ((memoryProperties.memoryProperties.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) != 0)
            {
                isSupported = true;
                break;
            }
    ...
    VmaPool MemoryAllocator::GetOrCreateBufferPool(VkDeviceSize size, const VmaMemoryUsage usage)
    {
        ...
        if (const auto it = m_BufferPools.find(size); it != m_BufferPools.end())
        {
        ...
        else
        {
            ...
            m_BufferPools[size] = MemoryPool(size, usage);
        }

        // Create new VMA pool
        MemoryPool &pool = m_BufferPools[size];
    ...
    VmaPool MemoryAllocator::GetOrCreateImagePool(VkDeviceSize size, VmaMemoryUsage usage)
    {
        ...
        // Check if pool for this size already exists
        if (const auto it = m_ImagePools.find(size); it != m_ImagePools.end())
        {
        ...
        {
            m_ImagePools[size] = MemoryPool(size, usage); // Create new pool entry for this size
        }

        // Create new VMA pool
        MemoryPool &pool = m_ImagePools[size];
    ...
    bool MemoryAllocator::CheckMemoryBudget() const
    {
		...
		// Guard: device/context might be gone during shutdown
		auto devRef = RenderContext::GetCurrentDevice();
		if (!devRef) return false;

		auto physRef = devRef->GetPhysicalDevice();
		if (!physRef || physRef->GetDevice() == VK_NULL_HANDLE)
			return false;
		...
		vkGetPhysicalDeviceMemoryProperties(physRef->GetDevice(), &memProps);
        ...
        for (uint32_t i = 0; i < memProps.memoryHeapCount; i++)
        {
            ...
            if (const float usagePercent = static_cast<float>(budgets[i].usage) / static_cast<float>(budgets[i].budget); usagePercent > m_MemoryWarningThreshold)
            {
        ...
        const bool isOverBudget = totalUsagePercent > m_MemoryWarningThreshold;
    ...
    VkDeviceSize MemoryAllocator::AlignBufferSize(VkDeviceSize size) const
    {
        ...
        if (m_CustomBufferAlignment > 0)
            // Round up to the next multiple of customBufferAlignment
            return ((size + m_CustomBufferAlignment - 1) / m_CustomBufferAlignment) * m_CustomBufferAlignment;
    ...
    {
        std::scoped_lock lock(const_cast<std::mutex &>(m_AllocationMutex));
        ...
        stats.allocCount = vmaStats.total.statistics.allocationCount;
        ...
        if (vmaStats.total.statistics.blockCount > 0)
            ...
            stats.fragRatio = 1.0f - static_cast<float>(vmaStats.total.statistics.allocationBytes) /
                                                  static_cast<float>(vmaStats.total.statistics.blockBytes);
        else
            stats.fragRatio = 0.0f;
    ...
    void MemoryAllocator::PrintDetailedStats() const
    {
        ...
        VkPhysicalDeviceMemoryProperties memProps;
        vkGetPhysicalDeviceMemoryProperties(RenderContext::GetCurrentDevice()->GetPhysicalDevice()->GetDevice(), &memProps);

        SEDX_CORE_INFO("----------- VULKAN MEMORY ALLOCATION STATS -----------");
        SEDX_CORE_INFO("Tag: {}", m_Tag);
        ...
        SEDX_CORE_INFO("Memory wasted (fragmentation): {}", FILE_SIZE(vmaStats.total.statistics.blockBytes - vmaStats.total.statistics.allocationBytes));
        ...
        for (uint32_t i = 0; i < memProps.memoryHeapCount; i++)
        {
            ...
            SEDX_CORE_INFO("Heap {}: {} - Size: {}, Used: {}({:.1f}%)", i, heapType, FILE_SIZE(budgets[i].budget), FILE_SIZE(budgets[i].usage), usagePercent);
        }
        ...
        for (uint32_t i = 0; i < memProps.memoryTypeCount; i++)
        {
            ...
            SEDX_CORE_INFO("Allocated: {}, Active allocations: {}", FILE_SIZE(memoryTypeStats[i].bytesAllocated - memoryTypeStats[i].bytesFreed), memoryTypeStats[i].currentAllocations);
        }
    ...
    void MemoryAllocator::FrameTick(const uint64_t frameCount) const
    {
    ...
    void MemoryAllocator::SetAllocationStrategy(const AllocationStrategy strategy)
    {
        ...
        if (strategy != m_CurrentStrategy)
        {
            SEDX_CORE_INFO_TAG("VulkanAllocator",
                               ...
                               m_CurrentStrategy == AllocationStrategy::DEFAULT          ? "Default"
                               : m_CurrentStrategy == AllocationStrategy::SPEED_OPTIMIZED ? "SpeedOptimized"
                                                                                       : "MemoryOptimized",
    ...
    MemoryAllocator::MemoryBudget MemoryAllocator::GetMemoryBudget() const
    {
        ...
        budget.isOverBudget = budget.usagePercentage > m_MemoryWarningThreshold;
    ...
    void MemoryAllocator::SetMemoryUsageWarningThreshold(float percentage)
    {
        ...
        m_MemoryWarningThreshold = percentage;
    ...
    void MemoryAllocator::SetBufferAlignment(VkDeviceSize alignment)
    {
        ...
        m_CustomBufferAlignment = alignment;
    ...
    void MemoryAllocator::Init(const uint32_t &apiVersion)
    {
        ...
        const VkPhysicalDevice physicalDevice = ctx->GetCurrentDevice()->GetPhysicalDevice()->GetDevice();
