...
* -------------------------------------------------------
*/
#include <SceneryEditorX/renderer/pipeline.h>
...
namespace SceneryEditorX
{
    ...
    Pipeline::Pipeline(const PipelineData &data) : m_PipelineSpecs(data)
    {
        SEDX_CORE_ASSERT(data.shader);
        SEDX_CORE_ASSERT(data.dstFramebuffer);
        Invalidate();
        Renderer::RegisterShader(data.shader, this);
    }

    Pipeline::~Pipeline()
    {
        Renderer::SubmitResourceFree(
            [pipeline = m_Pipeline, pipelineCache = m_PipelineCache, pipelineLayout = m_PipelineLayout]()
			{
    ...
    void Pipeline::Invalidate()
    {
        ...
        {
            // SEDX_CORE_WARN("[VulkanPipeline] Creating pipeline {0}", instance->m_Specification.DebugName);

            VkDevice device = RenderContext::GetCurrentDevice()->GetDevice();
            SEDX_CORE_ASSERT(instance->m_PipelineSpecs.shader);
            Ref<Shader> vulkanShader = Ref<Shader>(instance->m_PipelineSpecs.shader);
            Ref<Framebuffer> framebuffer = instance->GetSpecification().dstFramebuffer.As<Framebuffer>();
            ...
            inputAssemblyState.topology = Utils::GetPrimitiveTopology(instance->m_PipelineSpecs.topology);

            // Rasterization state
            VkPipelineRasterizationStateCreateInfo rasterizationState = {};
            rasterizationState.sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;
            rasterizationState.polygonMode = instance->m_PipelineSpecs.wireframe ? VK_POLYGON_MODE_LINE : VK_POLYGON_MODE_FILL;
            rasterizationState.cullMode = instance->m_PipelineSpecs.backfaceCulling ? VK_CULL_MODE_BACK_BIT : VK_CULL_MODE_NONE;
            rasterizationState.frontFace = VK_FRONT_FACE_CLOCKWISE;
            rasterizationState.depthClampEnable = VK_FALSE;
            rasterizationState.rasterizerDiscardEnable = VK_FALSE;
            rasterizationState.depthBiasEnable = VK_FALSE;
            rasterizationState.lineWidth = instance->m_PipelineSpecs.lineWidth; // this is dynamic
            ...
            depthStencilState.depthTestEnable = instance->GetSpecification().depthTest ? VK_TRUE : VK_FALSE;
            depthStencilState.depthWriteEnable = instance->GetSpecification().depthWrite ? VK_TRUE : VK_FALSE;
            depthStencilState.depthCompareOp = Utils::GetVulkanCompareOperator(instance->GetSpecification().depthOperator);
            ...
            VertexBufferLayout &vertexLayout = instance->GetSpecification().layout;
            VertexBufferLayout &instanceLayout = instance->GetSpecification().instanceLayout;
            VertexBufferLayout &boneInfluenceLayout = instance->GetSpecification().boneInfluenceLayout;
            ...
            VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, instance->m_PipelineCache, 1, &pipelineCreateInfo, nullptr, &instance->m_Pipeline));
            SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_PIPELINE, instance->GetSpecification().debugName, instance->m_Pipeline);
        });
    }

    Ref<Pipeline> Pipeline::Get()
    {
