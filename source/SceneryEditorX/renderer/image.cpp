...
namespace SceneryEditorX
{
	...
	int Image2D::GetClosestMipLevel(uint32_t width, uint32_t height) const
	{
		...
		int a = std::log2(Min(m_Specification.width, m_Specification.height));
		int b = std::log2(Min(width, height));
		return a - b;
	}
	...
	void Image2D::Invalidate_RenderThread()
	{
        ...
        if (m_Specification.format == RenderContext::GetCurrentDevice()->GetDepthFormat())
		...
		if (m_Specification.usage == Layout::ImageLayout::General)
		{
			...
			VkCommandBuffer commandBuffer = CommandManager::GetCommandBuffer(true);
			...
			CommandManager::Get()->FlushCmdBuffer();
		}
	...
	VkImageView Image2D::GetRenderThreadMipImageView(uint32_t mip)
	{
