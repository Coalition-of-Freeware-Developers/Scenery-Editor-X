...
namespace SceneryEditorX
{
    ...
    // Constructors
Viewport::Viewport(const float x,
                   const float y,
                   const float width,
                   const float height,
                   const float minDepth,
                   const float maxDepth)
    : m_Data{.x = x, .y = y, .width = width, .height = height, .minDepth = minDepth, .maxDepth = maxDepth}
{
}
...
{
    return m_Data.x == data.m_Data.x && m_Data.y == data.m_Data.y && m_Data.width == data.m_Data.width &&
           m_Data.height == data.m_Data.height && m_Data.minDepth == data.m_Data.minDepth &&
...
{
    return m_Data.x != 0.0f || m_Data.y != 0.0f || m_Data.width != 0.0f || m_Data.height != 0.0f ||
...
{
    m_Data.~ViewportData();
    new (&m_Data) ViewportData{.x = x,
                               .y = y,
                               .width = m_Data.width,
                               .height = m_Data.height,
                               .minDepth = m_Data.minDepth,
                               .maxDepth = m_Data.maxDepth};
    viewportResized = true;
}
...
{
    m_Data.~ViewportData();
    new (&m_Data) ViewportData{.x = m_Data.x,
                               .y = m_Data.y,
                               .width = width,
                               .height = height,
                               .minDepth = m_Data.minDepth,
