...
namespace SceneryEditorX
{
    ...
    RenderPassInput::RenderPassInput(Ref<UniformBuffer> uniformBuffer) : type(ResourceType::UniformBuffer)
    {
        input.reserve(1);
        input.push_back(uniformBuffer.As<RefCounted>());
    }

    RenderPassInput::RenderPassInput(Ref<UniformBufferSet> uniformBufferSet) : type(ResourceType::UniformBufferSet)
    {
        input.reserve(1);
        input.push_back(uniformBufferSet.As<RefCounted>());
    }

    RenderPassInput::RenderPassInput(Ref<StorageBuffer> storageBuffer) : type(ResourceType::StorageBuffer)
    {
        input.reserve(1);
        input.push_back(storageBuffer.As<RefCounted>());
    }

    RenderPassInput::RenderPassInput(Ref<StorageBufferSet> storageBufferSet) : type(ResourceType::StorageBufferSet)
    {
        input.reserve(1);
        input.push_back(storageBufferSet.As<RefCounted>());
    }

    RenderPassInput::RenderPassInput(Ref<Texture2D> texture) : type(ResourceType::Texture2D)
    {
        input.reserve(1);
        input.push_back(texture.As<RefCounted>());
    }

    RenderPassInput::RenderPassInput(Ref<TextureCube> texture) : type(ResourceType::TextureCube)
    {
        input.reserve(1);
        input.push_back(texture.As<RefCounted>());
    }

    RenderPassInput::RenderPassInput(Ref<Image2D> image) : type(ResourceType::Image2D)
    {
        input.reserve(1);
        input.push_back(image.As<RefCounted>());
    }

    void RenderPassInput::Set(Ref<UniformBuffer> uniformBuffer, uint32_t index)
    {
        type = ResourceType::UniformBuffer;
        if (input.size() <= index)
            input.resize(index + 1);
        input[index] = uniformBuffer.As<RefCounted>();
    }

    void RenderPassInput::Set(Ref<UniformBufferSet> uniformBufferSet, uint32_t index)
    {
        type = ResourceType::UniformBufferSet;
        if (input.size() <= index)
            input.resize(index + 1);
        input[index] = uniformBufferSet.As<RefCounted>();
    }

    void RenderPassInput::Set(Ref<StorageBuffer> storageBuffer, uint32_t index)
    {
        type = ResourceType::StorageBuffer;
        if (input.size() <= index)
            input.resize(index + 1);
        input[index] = storageBuffer.As<RefCounted>();
    }

    void RenderPassInput::Set(Ref<StorageBufferSet> storageBufferSet, uint32_t index)
    {
        type = ResourceType::StorageBufferSet;
        if (input.size() <= index)
            input.resize(index + 1);
        input[index] = storageBufferSet.As<RefCounted>();
    }

    void RenderPassInput::Set(Ref<Texture2D> texture, uint32_t index)
    {
        type = ResourceType::Texture2D;
        if (input.size() <= index)
            input.resize(index + 1);
        input[index] = texture.As<RefCounted>();
    }

    void RenderPassInput::Set(Ref<TextureCube> texture, uint32_t index)
    {
    ...
    void RenderPassInput::Set(Ref<Image2D> image, uint32_t index)
    {
    ...
    void RenderPassInput::Set(Ref<ImageView> image, uint32_t index)
    {
        type = ResourceType::Image2D;
        if (input.size() <= index)
            input.resize(index + 1);
            ...
            for (const auto& shaderDescriptor = shaderDescriptorSets[set]; auto&& [bname, wd] : shaderDescriptor.writeDescriptorSets)
			{
				...
				if (shaderDescriptor.imageSamplers.contains(binding))
				{
					...
					if (wd.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE || wd.descriptorType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER)
					{
						switch (dimension)
						{
							case 1: inputDecl.type = ResourceInputType::ImageSampler1D; break;
							case 2: inputDecl.type = ResourceInputType::ImageSampler2D; break;
							case 3: inputDecl.type = ResourceInputType::ImageSampler3D; break;
						}
					}
					else if (wd.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
					{
						switch (dimension)
						{
							case 1: inputDecl.type = ResourceInputType::StorageImage1D; break;
							case 2: inputDecl.type = ResourceInputType::StorageImage2D; break;
							case 3: inputDecl.type = ResourceInputType::StorageImage3D; break;
						}
	...
	void DescriptorSetManager::AddInput(std::string_view name, Ref<Texture2D> texture, uint32_t arrayIndex)
	{
        if (const RenderPassInputDeclaration *decl = GetInputDeclaration(name))
        {
            SEDX_CORE_VERIFY(arrayIndex < decl->count);
	...
	void DescriptorSetManager::AddInput(std::string_view name, Ref<TextureCube> textureCube, uint32_t arrayIndex)
	{
        if (const RenderPassInputDeclaration *decl = GetInputDeclaration(name))
        {
            SEDX_CORE_VERIFY(arrayIndex < decl->count);
            inputResources.at(decl->set).at(decl->binding).Set(textureCube, arrayIndex);
        }
        else
        {
            SEDX_CORE_WARN_TAG("Renderer", "[RenderPass ({})] Input {} not found", m_Specification.debugName, name);
	...
	void DescriptorSetManager::AddInput(std::string_view name, Ref<Image2D> image, uint32_t arrayIndex)
	{
        if (const RenderPassInputDeclaration *decl = GetInputDeclaration(name))
        {
            SEDX_CORE_VERIFY(arrayIndex < decl->count);
	...
	void DescriptorSetManager::AddInput(std::string_view name, Ref<ImageView> image, uint32_t arrayIndex)
	{
        if (const RenderPassInputDeclaration *decl = GetInputDeclaration(name))
        {
            SEDX_CORE_VERIFY(arrayIndex < decl->count);
	...
	void DescriptorSetManager::Bake()
	{
		...
		for (const auto& [set, setData] : inputResources)
		{
			...
			for (uint32_t frameIndex = 0; frameIndex < descriptorSetCount; frameIndex++)
			{
				...
				VkDescriptorSetAllocateInfo descriptorSetAllocInfo = Utils::DescriptorSetAllocInfo(&dsl);
				...
				for (const auto& [binding, input] : setData)
				{
					...
					switch (input.type)
					{
						...
						case ResourceType::UniformBufferSet:
						{
							...
							writeDescriptor.pBufferInfo = &buffer->Get(frameIndex).As<UniformBuffer>()->GetDescriptorInfo();
							storedWriteDescriptor.resourceHandles[0] = writeDescriptor.pBufferInfo->buffer;
						...
						case ResourceType::StorageBufferSet:
						{
							...
							// TODO: replace 0 with current frame in flight (i.e. create bindings for all frames)
							writeDescriptor.pBufferInfo = &buffer->Get(frameIndex).As<StorageBuffer>()->GetDescriptorInfo();
							storedWriteDescriptor.resourceHandles[0] = writeDescriptor.pBufferInfo->buffer;
						...
						{
							Ref<RenderResource> image = input.input[0].As<RenderResource>();
							// Defer if resource doesn't exist
							if (image == nullptr)
							{
								invalidatedInputResources[set][binding] = input;
			...
			for (const auto& [binding, input] : inputs)
			{
				switch (input.type)
				{
					case ResourceType::UniformBuffer:
					{
						...
						{
							const VkDescriptorBufferInfo& bufferInfo = input.input[0].As<UniformBuffer>()->GetDescriptorInfo();
					...
					case ResourceType::UniformBufferSet:
					{
						for (uint32_t frameIndex = 0; frameIndex < (uint32_t)writeDescriptorMap.size(); frameIndex++)
						{
							const VkDescriptorBufferInfo& bufferInfo = input.input[0].As<UniformBufferSet>()->Get(currentFrameIndex).As<UniformBuffer>()->GetDescriptorInfo();
							if (bufferInfo.buffer != writeDescriptorMap[currentFrameIndex].at(set).at(binding).resourceHandles[0])
							{
								invalidatedInputResources[set][binding] = input;
								break;
					...
					case ResourceType::StorageBufferSet:
					{
						for (uint32_t frameIndex = 0; frameIndex < (uint32_t)writeDescriptorMap.size(); frameIndex++)
						{
							const VkDescriptorBufferInfo& bufferInfo = input.input[0].As<StorageBufferSet>()->Get(currentFrameIndex).As<StorageBuffer>()->GetDescriptorInfo();
							if (bufferInfo.buffer != writeDescriptorMap[currentFrameIndex].at(set).at(binding).resourceHandles[0])
							{
								invalidatedInputResources[set][binding] = input;
								break;
					...
					case ResourceType::Texture2D:
					{
						for (size_t i = 0; i < input.input.size(); i++)
						{
							...
							if (vulkanTexture == nullptr)
								vulkanTexture = Renderer::GetWhiteTexture().As<Texture2D>(); // TODO: error texture
					...
					case ResourceType::TextureCube:
					{
						for (uint32_t frameIndex = 0; frameIndex < (uint32_t)writeDescriptorMap.size(); frameIndex++)
						{
                            const VkDescriptorImageInfo &imageInfo = input.input[0].As<TextureCube>()->GetDescriptorInfo();
