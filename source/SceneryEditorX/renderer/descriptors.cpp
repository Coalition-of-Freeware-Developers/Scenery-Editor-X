...
#include "SceneryEditorX/renderer/descriptor_set.h"

// -------------------------------------------------------

namespace SceneryEditorX
{
	DescriptorSet::DescriptorSet(const std::vector<Descriptors> &descriptors, DescriptorSetLayout *descriptor_set_layout,
                             const std::string &name)
	{
	    ...
	    // Allocate
	    {
	        VulkanDevice::AllocateDescriptorSet(m_Resource, descriptor_set_layout, descriptors);
	        VulkanDevice::SetResourceName(m_Resource, ResourceType::DescriptorSet, name);
	    }
	
	    Update(descriptors);
	}

	bool DescriptorSet::IsRefToResource(const void *resource)
	{
	    for (Descriptors &descriptor : m_Descriptors)
	    {
