	...
	*/
    ...
    void SwapChain::Create(uint32_t width, uint32_t height, bool vsync)
    {
        ...
        if (std::cmp_equal(surfaceInfo.currentExtent.width, -1) && std::cmp_equal(surfaceInfo.currentExtent.height, -1))
        {
            ...
            swapExtent.width = renderData.width;
            swapExtent.height = renderData.height;
        }
        else
        {
            swapExtent = surfaceInfo.currentExtent;
            renderData.width = surfaceInfo.currentExtent.width;
            renderData.height = surfaceInfo.currentExtent.height;
        }
		...
		for (auto &cmdBuffer : cmdBuffers)
            vkDestroyCommandPool(vkDevice, cmdBuffer.CommandPool, nullptr);
