...
namespace SceneryEditorX
{
    ...
    VkSampler CreateSampler(float maxLod)
    {
        ...
        VkSampler sampler;
        if (vkCreateSampler(vulkanDevice, &samplerInfo, nullptr, &sampler) != VK_SUCCESS)
        {
    ...
    */
    ...
    void GraphicsEngine::CreateInstance(const Ref<Window> &window)
    {
        ...
        if (enableValidationLayers)
        {
            ...
            createInfo.enabledLayerCount = static_cast<uint32_t>(layersInstance.validationLayer.size());
    ...
    */
    ...
    void GraphicsEngine::FramebufferResizeCallback(GLFWwindow *window, int width, int height)
    {
        ...
        if (static_cast<Window *>(glfwGetWindowUserPointer(window)))
        {
            ...
            EDITOR_LOG_WARN("Framebuffer Resize detected but no Window instance found"); /// Fallback if Window pointer isn't set
    }
