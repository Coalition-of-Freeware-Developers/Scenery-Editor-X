...
namespace SceneryEditorX
{
    ...
    public:
        ...
        static bool IsRequiredExtensionSupported(const std::vector<VkExtensionProperties> &availExtensions, const char *extension); /// Function to check if a required extension is supported
        static bool IsExtensionSupported(const char *extension);                                                                    /// Function to check if an extension is supported
        static bool CheckAPIVersion(uint32_t minVulkanVersion);                                                                     /// Function to check if Vulkan API version is compatible
        ...
        static bool CheckDeviceFeatures(const VkPhysicalDevice &device);                                                            /// Function to check the supported device features and if they are compatible.
        static bool IsDeviceCompatible(const VkPhysicalDevice &device);                                                             /// Function to check if a device is compatible
