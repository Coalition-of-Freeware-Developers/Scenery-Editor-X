...
namespace SceneryEditorX
{
    ...
    void SwapChain::InitSurface(GLFWwindow *window)
    {
        ...
        /// Create the surface
        if (glfwCreateWindowSurface(RenderContext::GetInstance(), window, nullptr, &surface) != VK_SUCCESS)
	...
	*/
    ...
    void SwapChain::Create(uint32_t *width, uint32_t *height, bool vsync)
    {
        ...
        for (size_t i = 0; i < swapChainFramebuffers.size(); i++)
        {
            ...
            VK_CHECK_RESULT(vkCreateFramebuffer(usedDevice, &framebufferInfo, nullptr, &swapChainFramebuffers[i]))
	...
	void SwapChain::Destroy()
    {
        ...
        for (auto &image : swapChainImages)
            vkDestroyImageView(device, image.resource->view, nullptr);
        swapChainImages.clear();
    ...
    void SwapChain::OnResize(uint32_t width, uint32_t height)
    {
        ...
        SEDX_CORE_INFO("Window resized to {}x{}", width, height);
        vkDeviceWaitIdle(device);
    ...
    void SwapChain::Present()
    {
        ...
        submitInfo.pCommandBuffers = &cmdBuffers[frameIndex].CommandBuffer;
        submitInfo.commandBufferCount = 1;

		VK_CHECK_RESULT(vkResetFences(vkDevice->GetDevice(), 1, &waitFences[frameIndex]))

		vkDevice->LockQueue();
		if (vkQueueSubmit(vkDevice->GetGraphicsQueue(), 1, &submitInfo, waitFences[frameIndex]) != VK_SUCCESS)
		{
		...
		if (result != VK_SUCCESS)
        {
			...
			{
				SEDX_CORE_INFO("Swap chain out of date, resizing...");
    ...
    uint32_t SwapChain::AcquireNextImage()
    {
        ...
        frameIndex = (frameIndex + 1) % renderData.framesInFlight;

		uint32_t swapImageIndex;
        if (VkResult result = fpAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphores[imageIndex], (VkFence)nullptr, &swapImageIndex); result != VK_SUCCESS)
        {
            if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR)
            {
                OnResize(viewportData.width, viewportData.height);
                VK_CHECK_RESULT(fpAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphores[imageIndex], (VkFence)nullptr, &swapImageIndex))
            }
        }

        return swapImageIndex;
	}
