...
namespace SceneryEditorX
{
	...
	struct Resource : RefCounted
	{
        ...
        const std::string &GetResourceName() const { return m_DebugName; }
		void SetObjectName(const std::string &name) { m_DebugName = name; }

        uint64_t GetResourceID() const { return m_ResourceID; }
		void SetResourceID(const uint64_t id) { m_ResourceID = id; }
        uint64_t GetResourceSize() const { return m_ResourceSize; }
		...
		*/
        ...
        uint64_t m_ResourceID = -1;
		/** @brief Size of the resource in bytes */
        uint64_t m_ResourceSize = 0;
    ...
    public:
		 ...
		 */
        virtual ResourceDescriptorInfo GetDescriptorResourceInfo() const = 0;
    };
