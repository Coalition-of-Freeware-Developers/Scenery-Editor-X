...
namespace SceneryEditorX
{
    ...
    */
	...
	{
	    SEDX_ASSERT(resource->GetResourceID() != static_cast<uint64_t>(-1), "Invalid Buffer Resource ID!");
	    return static_cast<uint32_t>(resource->GetResourceID());
	}
    ...
    Buffer CreateBuffer(uint64_t size, BufferUsageFlags usage, MemoryFlags memory, const std::string &name)
	{
		...
		if (usage & BufferUsage::Storage)
		{
			...
			resource->SetResourceID(static_cast<uint64_t>(bIndex));
		}
