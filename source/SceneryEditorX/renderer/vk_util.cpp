...
static std::vector<char> CompileShader(const std::filesystem::path &path)
{
    ...
    SEDX_CORE_TRACE("[ShaderCompiler] Command: {}", compile_string);
    SEDX_CORE_TRACE("[ShaderCompiler] Output:");
    while (system(compile_string))
    {
        SEDX_CORE_ERROR("[ShaderCompiler] Error! Press something to Compile Again");
        std::cin.get();
    }
    ...
    if (!file.is_open())
    {
        SEDX_CORE_ERROR("Failed to open file: '{}'", outpath);
    }
