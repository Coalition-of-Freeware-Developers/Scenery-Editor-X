...
GLOBAL const char *VK_DEVICE_TYPE_STRING(VkPhysicalDeviceType type)
{
	switch (type)
	{
...
GLOBAL const char *VK_COLOR_SPACE_STRING(VkColorSpaceKHR colorSpace)
{
	switch (colorSpace)
	{
...
GLOBAL const char *VK_QUEUE_FLAGS_STRING(VkQueueFlags flags)
{
	switch (flags)
	{
...
GLOBAL const char *VK_MEMORY_PROPERTY_FLAGS_STRING(VkMemoryPropertyFlags flags)
{
	switch (flags)
	{
...
GLOBAL const char *VK_DEBUG_SEVERITY_STRING(VkDebugUtilsMessageSeverityFlagBitsEXT severity)
{
	switch (severity)
	{
...
GLOBAL const char *VK_DEBUG_TYPE(VkDebugUtilsMessageTypeFlagsEXT type)
{
	switch (type)
	{
...
GLOBAL const char *VK_ERROR_STRING(VkResult result)
{
    switch (result)
    {
    case VK_SUCCESS:
...
GLOBAL const char *VK_VENDER_ID_STRING(uint32_t vendorID)
{
    switch (vendorID)
    {
    case 0x10DE:
...
void GPUInfoDump()
{
    ...
    EDITOR_LOG_TRACE("  Vendor: {0}", caps.Vendor);
    EDITOR_LOG_TRACE("  Device: {0}", caps.Device);
    EDITOR_LOG_TRACE("  Version: {0}", caps.Version);
}
 ...
 * @return 
 */
GLOBAL std::vector<char> CompileShader(const std::filesystem::path &path)
{
