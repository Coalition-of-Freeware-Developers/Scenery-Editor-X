...
namespace SceneryEditorX
{
    ...
    struct RenderPassInput
    {
    	...
    	RenderPassInput(const Ref<UniformBufferSet>& uniformBufferSet) : type(ResourceType::UniformBufferSet) { input.reserve(1); input.push_back(uniformBufferSet.As<RefCounted>()); }
    	...
    	RenderPassInput(const Ref<StorageBufferSet>& storageBufferSet) : type(ResourceType::StorageBufferSet) { input.reserve(1); input.push_back(storageBufferSet.As<RefCounted>()); }
    	...
    	void Set(const Ref<UniformBufferSet>& uniformBufferSet, uint32_t index = 0) { type = ResourceType::UniformBufferSet; if (input.size() <= index) input.resize(index+1); input[index] = uniformBufferSet.As<RefCounted>(); }
    	...
    	void Set(const Ref<StorageBufferSet>& storageBufferSet, uint32_t index = 0) { type = ResourceType::StorageBufferSet; if (input.size() <= index) input.resize(index+1); input[index] = storageBufferSet.As<RefCounted>(); }
    ...
    public:
        ...
        static void Pass_Clear(CommandManager* cmd_list);
