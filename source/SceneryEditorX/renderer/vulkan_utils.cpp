...
namespace SceneryEditorX::Utils
{
	...
	const char* DeviceTypeString(const VkPhysicalDeviceType type)
	{
	    switch (type)
	    {
	 ...
	 */
	const char* ColorSpaceString(const VkColorSpaceKHR colorSpace)
	{
	    switch (colorSpace)
	    {
	 ...
	 */
	const char* QueueFlagsString(const VkQueueFlags flags)
	{
	    switch (flags)
	    {
	 ...
	 */
	const char* MemoryPropertyFlagsString(VkMemoryPropertyFlags flags)
	{
	    switch (flags)
	    {
	 ...
	 */
	const char* DebugSeverityString(const VkDebugUtilsMessageSeverityFlagBitsEXT severity)
	{
	    switch (severity)
	    {
	 ...
	 */
	const char* DebugType(const VkDebugUtilsMessageTypeFlagsEXT type)
	{
	    switch (type)
	    {
	...
	* @brief Get the number of bytes per pixel for a given format.
	 ...
	 */
    const char *VulkanErrorString(const VkResult errorCode)
    {
        switch (errorCode)
        {
        case VK_SUCCESS:
	 ...
	 */
    const char *VulkanFormatString(const VkFormat format)
    {
        switch (format)
        {
        ...
        case VK_FORMAT_A1B5G5R5_UNORM_PACK16:
            return "VK_FORMAT_A1B5G5R5_UNORM_PACK16"; /// ONLY IN VULKAN 1.4+
        case VK_FORMAT_A8_UNORM:
            return "VK_FORMAT_A8_UNORM"; /// ONLY IN VULKAN 1.4+
	...
	VkCompareOp GetDepthCompareOperator(const CompareFunc compareOp)
	{
	    switch (compareOp)
	    {
			...
			case CompareFunc::LessOrEqual:		return VK_COMPARE_OP_LESS_OR_EQUAL;
			case CompareFunc::Greater:			return VK_COMPARE_OP_GREATER;
			case CompareFunc::GreaterOrEqual:	return VK_COMPARE_OP_GREATER_OR_EQUAL;
			case CompareFunc::Equal:			return VK_COMPARE_OP_EQUAL;
			case CompareFunc::Always:			return VK_COMPARE_OP_ALWAYS;
	        case CompareFunc::MaxEnum:
                break;
        }
    ...
    /**
     * @brief Check if a given Vulkan format is a depth format.
     * 
     * This is a compile-time constant check against known depth formats.
     * Does not require a device context to be initialized.
     * 
