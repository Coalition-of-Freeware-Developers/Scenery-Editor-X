...
static const float PI = 3.14159265358979323846f;			// Pi constant (π)
static const float TWO_PI = 2.0f * PI;						// Two times pi (2π)
static const float HALF_PI = PI * 0.5f;					// Half pi (π/2)
static const float DEG_TO_RAD = PI / 180.0f;				// Degrees to radians conversion factor
static const float RAD_TO_DEG = 180.0f / PI;				// Radians to degrees conversion factor
static const float E = 2.71828182845904523536f;			// Euler's number (e)
static const float SQRT2 = 1.41421356237309504880f;		// Square root of 2
static const float SQRT3 = 1.73205080756887729352f;		// Square root of 3
static const float INV_PI = 1.0f / PI;						// 1 / π
static const float INV_TWO_PI = 1.0f / TWO_PI;				// 1 / (2π)
static const float PI_OVER_4 = PI * 0.25f;					// π/4
static const float PI_OVER_6 = PI / 6.0f;					// π/6
static const float LN2 = 0.69314718055994530942f;			// Natural log of 2
static const float LN10 = 2.30258509299404568402f;			// Natural log of 10
static const float EPSILON_F = 1.1920928955078125e-07f;	// Float epsilon (FLT_EPSILON)
static const float HUGE_F = 3.402823466e+38f;				// Max float (approx)
static const float TINY_F = 1.175494351e-38f;				// Min positive normal float (approx)
...
// Panos Karabelas take on the interleaved gradient function from Jimenez 2014 http://goo.gl/eomGso
