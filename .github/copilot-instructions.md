...
- **Documentation:** Comprehensive inline comments and Doxygen-style documentation for public APIs
- **Testing:** Unit tests for core systems and components using Catch2
- **Version Control:** Git with feature branches, pull requests, and code reviews
- **C++ RAII Principles:** Use RAII for resource management and avoid manual memory management where possible
...
- **Vulkan Resource Management:** Use RAII wrappers for Vulkan resources and proper error handling
- **Performance Profiling:** Use `SEDX_PROFILE_*` macros to mark critical sections for profiling with Tracy
- **Consistent Naming Conventions:** Follow established naming conventions for classes, functions, variables, and macros for clarity and consistency
...
## Vulkan API Usage Guidelines

### Synchronization Primitives
- Fences are used to signal work completion from GPU to CPU. We use them when we need to make sure that a resource used by both GPU and CPU is free to be modified on the CPU.
- Binary Semaphores are used to control access to resources on the GPU-side (only). We use them to ensure proper ordering for things like presentation.
- Pipeline barriers are used to control resource access within a GPU queue. We use them for layout transitions of images.
