...
/// <summary>
/// Converts a Vulkan VkResult error code to a human-readable string.
/// </summary>
/// <param name="result">The VkResult error code.</param>
...
namespace vkw
{
    ...
    struct Context
    {
        ...
        std::string applicationName = "Scenery Editor X";
        std::string engineName = "SEDX";
    ...
    Buffer CreateBuffer(uint32_t size, BufferUsageFlags usage, MemoryFlags memory, const std::string &name)
    {
        ...
        if (usage & BufferUsage::Storage)
        {
            ...
            write.dstBinding = SEDX_BINDING_BUFFER;
    ...
    Image CreateImage(const ImageDesc &desc)
    {
        ...
        if (desc.usage & ImageUsage::Sampled)
        {
            ...
            write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            write.dstSet = _ctx.bindlessDescriptorSet;
            write.dstBinding = SEDX_BINDING_TEXTURE;
    ...
    TLAS CreateTLAS(uint32_t maxInstances, const std::string &name)
    {
        ...
        tlasRes->instancesBuffer = vkw::CreateBuffer(sizeof(VkAccelerationStructureInstanceKHR) * maxInstances,
                                                     vkw::BufferUsage::AccelerationStructureInput,
                                                     vkw::Memory::GPU);
        ...
        _ctx.vkGetAccelerationStructureBuildSizesKHR(device,
                                                     VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
                                                     &tlasRes->buildInfo,
                                                     &maxInstances,
                                                     &sizeInfo);
    
        tlasRes->buffer = vkw::CreateBuffer(sizeInfo.accelerationStructureSize,
                                            vkw::BufferUsage::AccelerationStructure,
                                            vkw::Memory::GPU);
        if (_ctx.asScratchBuffer.size < sizeInfo.buildScratchSize)
        {
            _ctx.asScratchBuffer = vkw::CreateBuffer(sizeInfo.buildScratchSize,
                                                     vkw::BufferUsage::Address | vkw::BufferUsage::Storage,
                                                     vkw::Memory::GPU);
        }
        ...
        writeBindlessAccelerationStructure.dstBinding = SEDX_BINDING_TLAS;
    ...
    BLAS CreateBLAS(const BLASDesc &desc)
    {
        ...
        res->buffer = vkw::CreateBuffer(res->sizeInfo.accelerationStructureSize,
                                        vkw::BufferUsage::AccelerationStructure,
                                        vkw::Memory::GPU);
        ...
        {
            _ctx.asScratchBuffer =
                vkw::CreateBuffer(res->sizeInfo.buildScratchSize, vkw::BufferUsage::Storage, vkw::Memory::GPU);
        }
    ...
    void Context::CreatePipeline(const PipelineDesc &desc, Pipeline &pipeline)
    {
        ...
        vkRes =
            vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, allocator, &pipeline.resource->pipeline);
    ...
    void CmdBuildTLAS(TLAS &tlas, const std::vector<BLASInstance> &blasInstances)
    {
        ...
        for (int i = 0; i < instances.size(); i++)
        {
            ...
            instances[i].accelerationStructureReference =
                _ctx.vkGetAccelerationStructureDeviceAddressKHR(device, &addressInfo);
    ...
    void CmdBindPipeline(Pipeline &pipeline)
    {
        ...
        vkCmdBindDescriptorSets(cmd.buffer,
                                (VkPipelineBindPoint)pipeline.point,
                                pipeline.resource->layout,
                                0,
                                1,
                                &_ctx.bindlessDescriptorSet,
                                0,
                                nullptr);
    ...
    void BeginCommandBuffer(Queue queue)
    {
        ...
        {
            vkGetQueryPoolResults(_ctx.device,
                                  cmd.queryPool,
                                  0,
                                  cmd.timeStamps.size(),
                                  cmd.timeStamps.size() * sizeof(uint64_t),
                                  cmd.timeStamps.data(),
                                  sizeof(uint64_t),
            ...
            for (int i = 0; i < cmd.timeStampNames.size(); i++)
            {
                ...
                _ctx.timeStampTable[cmd.timeStampNames[i]] =
                    float(end - begin) * _ctx.physicalProperties.limits.timestampPeriod / 1000000.0f;
            }
    ...
    void Context::CreateDevice()
    {
        ...
        vkGetAccelerationStructureBuildSizesKHR =
            (PFN_vkGetAccelerationStructureBuildSizesKHR)vkGetDeviceProcAddr(device,
                                                                             "vkGetAccelerationStructureBuildSizesKHR");
        ...
        VkDescriptorPoolSize imguiPoolSizes[] = {{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1000},
        ...
        // create bindless resources
        {
            ...
            texturesBinding.binding = SEDX_BINDING_TEXTURE;
            ...
            storageBuffersBinding.binding = SEDX_BINDING_BUFFER;
            ...
            accelerationStructureBinding.binding = SEDX_BINDING_TLAS;
        ...
        asScratchBuffer = vkw::CreateBuffer(initialScratchBufferSize,
                                            vkw::BufferUsage::Address | vkw::BufferUsage::Storage,
                                            vkw::Memory::GPU);
        ...
        // command pool
        {
            ...
            for (int q = 0; q < Queue::Count; q++)
            {
                ...
                for (int i = 0; i < framesInFlight; i++)
                {
                    ...
                    queue.commands[i].staging = CreateBuffer(stagingBufferSize,
                                                             BufferUsage::TransferSrc,
                                                             Memory::CPU,
                                                             "StagingBuffer" + std::to_string(q) + "_" + std::to_string(i));
                    vkMapMemory(device,
                                queue.commands[i].staging.resource->memory,
                                0,
                                stagingBufferSize,
                                0,
                                (void **)&queue.commands[i].stagingCpu);
    ...
    void AcquireImage()
    {
        ...
        auto res = vkAcquireNextImageKHR(_ctx.device,
                                         _ctx.swapChain,
                                         UINT64_MAX,
                                         _ctx.imageAvailableSemaphores[_ctx.swapChainCurrentFrame],
                                         VK_NULL_HANDLE,
                                         &imageIndex);
    ...
    /// <summary>
    /// Checks if a given format is supported for a specific tiling and feature set.
    /// </summary>
    /// <param name="format">The Vulkan format to check.</param>
    /// <param name="tiling">The tiling mode (linear or optimal).</param>
    /// <param name="features">The format feature flags to check against.</param>
    /// <returns>True if the format is supported with the given tiling and features, false otherwise.</returns>
    ...
    /// </summary>
    /// <param name="formats">A vector of available VkSurfaceFormatKHR formats.</param>
    /// <returns>The chosen VkSurfaceFormatKHR format.</returns>
    ...
    /// <summary>
    /// Chooses the best available present mode from a list of present modes.
    /// </summary>
    /// <param name="presentModes">A vector of available VkPresentModeKHR present modes.</param>
    /// <returns>The chosen VkPresentModeKHR present mode.</returns>
    ...
    /// <summary>
    /// Chooses the extent of the swap chain images.
    /// </summary>
    /// <param name="capabilities">The surface capabilities.</param>
    /// <param name="width">The desired width of the swap chain images.</param>
    /// <param name="height">The desired height of the swap chain images.</param>
    /// <returns>The chosen extent of the swap chain images.</returns>
    ...
    /// <param name="dstOfsset">The offset in the destination buffer where the data should be copied.</param>
    ...
    /// <param name="size">The size of the data to copy.</param>
    /// <param name="dstOffset">The offset in the destination buffer where the data should be copied.</param>
    /// <param name="srcOffset">The offset in the source buffer where the data should be copied from.</param>
