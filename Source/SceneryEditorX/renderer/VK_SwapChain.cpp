...
#endif
{
    ...
    {
        VKEngine::tools::exitFatal("Could not fetch function pointer for the headless extension!", -1);
    }
    ...
    if (err != VK_SUCCESS)
    {
        VKEngine::tools::exitFatal("Could not create surface!", err);
    }
    ...
    {
        VKEngine::tools::exitFatal("Could not find a graphics and/or presenting queue!", -1);
    }

    if (graphicsQueueNodeIndex != presentQueueNodeIndex)
    {
        VKEngine::tools::exitFatal("Separate graphics and presenting queues are not supported yet!", -1);
    }
    ...
    std::vector<VkFormat> preferredImageFormats = {VK_FORMAT_B8G8R8A8_UNORM,
...
void VulkanSwapChain::createDirect2DisplaySurface(uint32_t width, uint32_t height)
{
    ...
    if (!foundMode)
    {
        VKEngine::tools::exitFatal("Can't find a display and a display mode!", -1);
        return;
    }
    ...
    if (bestPlaneIndex == UINT32_MAX)
    {
        VKEngine::tools::exitFatal("Can't find a plane for displaying!", -1);
        return;
    }
    ...
    {
        VKEngine::tools::exitFatal("Failed to create surface!", result);
    }
