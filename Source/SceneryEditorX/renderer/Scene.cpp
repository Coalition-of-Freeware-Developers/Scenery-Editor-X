...
namespace Scene
{
    ...
    void UpdateResources()
    {
        ...
        for (int i = 0; i < modelEntities.size(); i++)
        {
            ...
            {
                models[i].material.metallicRoughnessMap =
                    AssetManager::images[model->block.material.metallicRoughnessMap].RID();
            }
    ...
    /* @brief Sets the collection for the given entity. If the entity already has a parent, it is removed from the current collection.
    * If the provided collection is null, the entity is added to the root collection.
    * @param entity The entity to be added to the collection.
    * @param collection The collection to which the entity will be added. If null, the entity is added to the root collection.
    */
    ...
    void OnImgui()
    {
        bool controlPressed = Window::IsKeyDown(GLFW_KEY_LEFT_CONTROL) ||
    ...
    /// @brief Renders a transform gizmo for manipulating the given transform.
    ...
    void RenderTransformGizmo(Transform &transform)
    {
        ...
        ImGuizmo::DecomposeMatrixToComponents(glm::value_ptr(transform.transform),
    ...
    /**
     * @brief Accepts a mesh payload from a drag-and-drop operation.
     * 
     * This function checks if a drag-and-drop target is active and accepts a payload of type "mesh".
     * If a payload is accepted, it extracts the file path from the payload data and asynchronously
     * loads the models from the specified path using the AssetManager.
     */
    void AcceptMeshPayload()
    {
    ...
    /// @brief Accepts a texture payload from a drag-and-drop operation and updates the provided texture ID.
    /// @param textureID Reference to the texture ID to be updated.
    void AcceptTexturePayload(int &textureID)
    {
        if (ImGui::BeginDragDropTarget())
        {
