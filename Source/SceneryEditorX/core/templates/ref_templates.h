...
#include <atomic>
#include <cstddef>
#include <SceneryEditorX/renderer/vk_cmd_queue.h>
#include <type_traits>
...
namespace SceneryEditorX
{
	...
	{
	public:
	    Ref() : InstancePtr_(nullptr) {}
	
	    Ref(std::nullptr_t n) : InstancePtr_(nullptr) {}
	
	    Ref(T *instance) : InstancePtr_(instance)
	    {
	    ...
	    Ref(const Ref<T> &other) : InstancePtr_(other.InstancePtr_)
	    {
	        IncRef();
	    }
	...
	public:
	    ...
	    bool IsValid() const
	    {
	        return InstancePtr_ ? RefUtils::IsLive(InstancePtr_) : false;
	    }
	    operator bool() const
	    {
	        return IsValid();
