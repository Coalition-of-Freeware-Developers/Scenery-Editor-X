    ...
    try
    {
        ...
        while (ObjFile.good())
        {
            ...
            else if (strCommand == "IDX10")
            {
                ...
                //Read the args
                ssLine >> strIndicies[0] >> strIndicies[1] >> strIndicies[2] >> strIndicies[3] >> strIndicies[4] >>
                    strIndicies[5] >> strIndicies[6] >> strIndicies[7] >> strIndicies[8] >> strIndicies[9];

                //Push 'em back
                for (int i = 0; i < 10; i++)
                {
                    Indices.push_back(stoi(strIndicies[i]));
                }
            }
